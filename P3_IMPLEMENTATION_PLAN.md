...
Текущий `npm run verify` проходит: lint, 26 unit-тестов и production build.
...
| Net worth, активы и цели | NOT STARTED | Соответствующей модели и UI нет | Assets/liabilities valuation, history, goals и net-worth report |
| Настраиваемый dashboard | PARTIAL | Можно скрыть только debts; настройки локальные в browser localStorage | Полный набор widgets, порядок/размер, серверная синхронизация по пользователю |
...
| Восстановление JSON-бэкапа | NOT STARTED | Реализован только export backup | Version validation, dry-run preview, transactional restore и rollback |
| Массовое редактирование | NOT STARTED | В списке операций нет selection/bulk actions | Bulk select/edit/tag/category/status с permission checks и atomic/batched RPC |
...
3. Закрыть release evidence privacy-first распознавания чеков; balance history реализована и проверена.
4. Закрыть P2 platform: Web Push/email, PWA/offline и transactional restore.
5. Закрыть P2 product: net worth/assets/goals, полноценный dashboard и bulk edit.
6. Добавить E2E suite и провести повторную приёмку всех 15 пунктов; DB integration/RLS suite для P1 уже добавлена.

Только итог `15/15 DONE` открывает P3. Исключение возможно лишь через письменное изменение scope P1/P2, а не через перенос незавершённой функции в backlog.

### Что отсутствует для P3

1. Контрагенты сейчас существуют только как текст в долгах и описаниях.
2. Существующие `accounts` — денежные счета, а не invoices/bills.
3. Нет документа, строк документа, нумерации, срока оплаты и распределения платежей.
4. Проекты и центры затрат нельзя назначить операции или части операции.
...
9. Клиентский `usePermissions` и политики БД уже расходятся: UI разрешает удаление операций только Owner/Admin, а RLS разрешает Member удалить собственную операцию.
10. Нет интеграционных тестов схемы, RLS, конкурентных переходов статусов и неизменяемости аудита.
