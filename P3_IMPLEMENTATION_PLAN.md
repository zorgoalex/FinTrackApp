...
Текущая ветка проходит: lint, 46 unit-тестов, production build и 14 новых pgTAP-тестов PWA/push поверх ранее подтверждённых 127 тестов. Новая миграция и suite проверены напрямую через `psql` против тестового Supabase в транзакции с `ROLLBACK`. Browser smoke подтвердил загрузку application shell без работающего HTTP-сервера.
...
- Push/email delivery — `PARTIAL`: реализация готова, нужны secrets, deployment и delivery smoke;
- PWA/offline-first — `PARTIAL`: реализация готова, нужны deployment и авторизованный E2E.
...
| Push/email/in-app уведомления | PARTIAL | Реализованы in-app, Telegram, Web Push subscriptions/service worker, Resend email, digest, retry и delivery status; DB/RLS suite проходит | Задать VAPID/Resend secrets, применить миграцию, развернуть dispatcher и подтвердить доставку на реальных browser/email endpoints |
...
| PWA и offline-first добавление расходов | PARTIAL | Реализованы manifest, service worker, offline shell, IndexedDB-кеш workspace/счетов/категорий, очередь расходов, автоматическая синхронизация, серверная идемпотентность и явные конфликты; unit/build/DB и offline shell smoke проходят | Развернуть production build и пройти авторизованный E2E: install → offline expense → reconnect → ровно одна операция |
...
4. Закрыть P2 platform: реализация Web Push/email и PWA/offline готова; остаются deployment и сквозная release-приёмка. Transactional restore DONE.
...
Предусловие: P1/P2 readiness gate имеет статус `14/14 DONE`. Все оценки ниже начинаются после прохождения этого gate.
