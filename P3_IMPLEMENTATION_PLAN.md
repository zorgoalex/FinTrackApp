...
Текущая проверка проходит: lint, 40 unit-тестов, production build и 141 удалённый pgTAP-тест в 9 suites. Assets/dashboard E2E подтверждён на тестовом Supabase-проекте для ролей Owner и Member, включая пересчёт net worth и сохранение dashboard preferences после reload.
...
Текущий честный статус readiness gate: **13/15 DONE**. Незавершённые пункты относятся к P2 platform:

- Push/email delivery — `PARTIAL`;
- PWA/offline-first — `NOT STARTED`.

Этап **P2 Product Completion — assets/net worth и server-synced dashboard** завершён. Это не открывает P3 до завершения двух platform-пунктов.
...
| Net worth, активы и цели | DONE | Миграция `20260713000000_net_worth_and_dashboard.sql`, RLS, valuation history, goals, FX-aware report/history RPC, `/assets` и dashboard widget применены к тестовому проекту | Подтверждено 24 профильными DB/RLS tests, полным regression suite и Owner/Member E2E |
...
5. ~~Закрыть P2 product: net worth/assets/goals, полноценный dashboard и bulk edit.~~ DONE.
