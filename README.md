...
- **Безопасный импорт** — PDF, фото/скриншоты и CSV; self-hosted GLM-OCR с явным согласием, локальный резерв, маскирование персональных данных, предпросмотр и защита от дублей
- **Чеки без хранения оригиналов** — изображение временно передаётся на собственный OCR-сервер либо обрабатывается локальным fallback; исходный файл, имя и raw OCR не сохраняются, в БД попадают только подтверждённые нормализованные данные и опциональный маскированный комментарий позиций (принятый P1 scope: [`DOCUMENT_IMPORT_PRIVACY.md`](DOCUMENT_IMPORT_PRIVACY.md))
...
### Высокоточное распознавание чеков

Основной OCR для фотографий и сканированных PDF — self-hosted `GLM-OCR-Q8_0`,
постоянно загруженный в `llama-server`. Браузерные PP-OCR/Tesseract оставлены
только как явно обозначенный резерв. Развёртывание сервиса описано в
`services/receipt-ocr/README.md`.

После запуска HTTPS-шлюза добавьте его публичный URL во фронтенд:

```bash
VITE_RECEIPT_OCR_URL=https://ocr.your-domain.example
```

Это не секрет: браузер обращается к URL с текущим Supabase JWT, а сервер
дополнительно проверяет активное членство в выбранном пространстве. Модель,
изображения и OCR-текст не передаются OpenRouter или другому внешнему AI API.
Изображение и сырой текст существуют на OCR-сервере только в памяти запроса.
