...
export default function MfaSettings() {
  const { requireTotpVerification } = useAuth();
  const [state, setState] = useState({ loading: true, factors: [], error: '', message: '' });
  ...
  const removeFactor = async (factor) => {
    ...
    try {
      const confirmed = await requireTotpVerification('Отключение TOTP требует свежего кода из приложения-аутентификатора');
      if (!confirmed) return;
    ...
    <div>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm text-gray-500">Необязательная защита входа одноразовым кодом из приложения-аутентификатора.</p>
        </div>
      </div>
      {state.loading ? <p className="mt-3 text-sm text-gray-500">Проверяем факторы…</p> : (
        <div className="mt-3 space-y-2">
          {state.factors.length === 0 ? (
            <p className="rounded-xl bg-gray-50 p-3 text-sm text-gray-600 dark:bg-gray-900/50 dark:text-gray-300">TOTP не подключён. Вы можете включить его добровольно, если хотите дополнительно защищать вход.</p>
            ...
            <div key={factor.id} className="flex items-center justify-between gap-3 rounded-xl border border-gray-200 p-3 dark:border-gray-700">
              ...
              <button type="button" onClick={() => removeFactor(factor)} disabled={removingId === factor.id} className="btn-secondary min-h-11 min-w-11 p-2 text-red-600 disabled:opacity-40" aria-label="Удалить TOTP-фактор"><Trash2 size={16} /></button>
            </div>
          ))}
