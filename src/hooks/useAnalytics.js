...
export default function useAnalytics(workspaceIds, { dateFrom, dateTo } = {}) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let query = supabase
        .from('operations')
        .select('id, workspace_id, type, amount, currency, exchange_rate, base_amount, description, operation_date, category_id, user_id, created_at, transfer_group_id, transfer_direction, operation_allocations(id, category_id, counterparty_id, amount, base_amount, position), tags:operation_tags(tag_id, tags(id, name, color))')
        .in('workspace_id', ids)
